...
export function detectAnimalType(productName: string): keyof typeof ANIMAL_ICONS {
  ...
  if (name.includes('pássaro') || name.includes('bird') || name.includes('canário') || name.includes('papagaio')) {
    return 'birds';
  }
 ...
 */
export function generateFallbackImages(productName: string, category?: string, count: number = 3): string[] {
 ...
 */
export function optimizeUnsplashUrl(url: string, width: number = 400, height: number = 300): string {
