...
export class ProductController {
  ...
  async getProductsByPage(
    ...
    if (!page || !pageSize) throw new Error("Missing query parameters");
  ...
  async getProductsByPageAndCategory(
    @Query("page") page: number,
    @Query("pageSize") pageSize: number,
    @Query("category") productCategory: string,
  ): Promise<ReadProductDTO[]> {
    if (!page || !pageSize || !productCategory)
      throw new Error("Missing query parameters");

    try {
      return await this.productService.getProductsByPageAndCategory(
        page,
        pageSize,
        productCategory,
      );
