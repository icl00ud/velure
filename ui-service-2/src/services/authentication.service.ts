...
import { ILoginUser, ILoginResponse, IRegisterUser, Token } from '../types/user.types';
...
class AuthenticationService {
  ...
  async logout(refreshToken: string): Promise<boolean> {
    try {
      const response = await fetch(`${configService.authenticationServiceUrl}/logout/${refreshToken}`, {
        method: 'DELETE',
      });
